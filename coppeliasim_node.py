...
class CoppeliaSimNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.create_subscription(
            TwistStamped,         # Tipo de mensaje
            '/cmd_vel',           # Nombre del tópico
            self._next_step_callback,  # Callback que procesa el mensaje
        ...
        self.odom_publisher = self.create_publisher(Odometry, '/odom', 10)  # 10 is the queue size
        self.us_scan_publisher = self.create_publisher(RangeScan, '/us_scan', 10)
    ...
    def _next_step_callback(self, cmd_vel_msg: TwistStamped, pose_msg: PoseStamped = PoseStamped()):
        ...
        v : float = cmd_vel_msg.twist.linear.x  # Extraer velocidad lineal
        w : float = cmd_vel_msg.twist.angular.z  # Extraer velocidad angular
    ...
    def _publish_odometry(self, z_v: float, z_w: float) -> None:
        ...
        # Configurar las velocidades lineal y angular
    ...
    def _publish_us(self, z_us: list[float]) -> None:
        ...
        # Crear el mensaje de sensores ultrasónicos
        us_scan_msg = RangeScan()

        # Asignar las distancias al campo 'ranges'
        us_scan_msg.ranges = z_us

        # Publicar el mensaje
        self.us_scan_publisher.publish(us_scan_msg)
